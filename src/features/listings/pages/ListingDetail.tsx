...
export default function ListingDetail() {
  ...
  const specs = [
    ...(listing.condition
      ? [{ label: 'Condition', value: listing.condition.charAt(0) + listing.condition.slice(1).toLowerCase() }]
      : []),
  ...
  return (
    <>
        ...
        <div className="ws-listing">
          <div className="ws-listing__main">
            <div className="ws-listing__gallery">
              <ListingGallery key={listing.id} images={images} name={listing.name} />
            </div>

            <div className="ws-listing__body">
              <section className="ws-detail__section" aria-labelledby="listing-about">
                <h2 className="ws-h2" id="listing-about">About this item</h2>
                <p className="ws-body ws-muted ws-listing__desc">{listing.description}</p>
              ...
              {specs.length > 0 && (
                <section className="ws-detail__section" aria-labelledby="listing-details">
                  <h2 className="ws-h2" id="listing-details">Details</h2>
                  <p className="ws-listing__hint">Provided by the seller. Ask them to confirm anything that matters to you.</p>
              ...
              {listing.variants.length > 0 && (
                <section className="ws-detail__section" aria-labelledby="listing-options">
                  <h2 className="ws-h2" id="listing-options">Available options</h2>
                  <p className="ws-listing__hint">Each option has its own price. Say which one you want when you message the seller.</p>
                  <dl className="ws-spec">
                    {listing.variants.map((v, i) => (
                      <div className="ws-spec__row" key={v.id ?? i}>
                        ...
                        <dd className="ws-num">
                          ...
                          {v.isAvailable === false && (
                            <span className="ws-badge ws-badge--danger" style={{ marginLeft: 'var(--ws-space-2)' }}>
                              Sold out
                            </span>
                          )}
                        </dd>
      ...
      <div className="ws-actionbar">
        ...
        <button type="button" className="ws-btn ws-btn--primary" onClick={scrollToContact}>
          Message seller
        </button>
      </div>
