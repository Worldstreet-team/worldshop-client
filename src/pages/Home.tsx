...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const cachedFeatured = productCache.featuredProducts?.data;
      if (cachedFeatured) {
      ...
      try {
        // Always fetch (from cache if fresh, network if stale)
        const [products, allProductsResult] = await Promise.all([
          productCache.getFeaturedProducts(),
          productCache.getProducts({ limit: 50 }),
        ]);
  ...
  const dealSaving = dealProduct
    ? Math.round(dealProduct.basePrice - (dealProduct.salePrice || dealProduct.basePrice))
    : 0;
  ...
  // First 4 categories for display
  const displayedCategories = categoryStore.categories.slice(0, 4);
            ...
            {totalSlides > 1 && (
              <div className="slider-pagination">
                {heroSlides.map((_, index) => (
                  <button
                    key={index}
                    className={`pagination-bar ${index === currentBanner ? 'active' : ''}`}
          ...
          <div className="deals-tabs-row">
            ...
            <div className="deal-card">
              <div className="deal-card-header">
                ...
                {dealSaving > 0 && (
                  <div className="deal-save-badge">
                    ...
                    <span className="deal-save-amount">
                      {formatPrice(dealSaving)}
                    </span>
                  </div>
                )}
              </div>
              {isLoading ? (
                ...
                <>
                  <Link
                    to={`/products/${dealProduct.slug}`}
                    className="deal-card-image"
                  >
                    <img
                      src={
                        dealProduct.images?.[0]?.url ||
                        '/images/320X300/img1.jpg'
                      }
                  ...
                  <div className="deal-card-pricing">
                    {dealProduct.salePrice ? (
                      <>
                        <del className="deal-card-original">
                          {formatPrice(dealProduct.basePrice)}
                        </del>
                        <ins className="deal-card-sale">
                  ...
                  <div className="deal-card-stock">
                    <div className="deal-card-stock-labels">
                      <span>
                        Available: <strong>{dealProduct.stock}</strong>
                      </span>
                      <span>
                    ...
                    <div className="deal-card-progress">
                      <span
                        className="deal-card-progress-fill"
                        style={{ width: '30%' }}
                      />
                    </div>
                  </div>
                  <div className="deal-card-countdown">
                    <p className="deal-card-countdown-label">
            ...
            <div className="tabbed-products">
              <div className="tabs-nav">
                ...
                <button
                  className={`tab-btn ${activeTab === 'on-sale' ? 'active' : ''}`}
              ...
              <div className="tab-products-grid">
                ...
                {!isLoading && tabProducts.length === 0 && (
                  ...
                  >
                    ...
                    <div className="category-info">
                      ...
                      <span className="product-count">
                        {(category as { productCount?: number }).productCount ||
                          0}{' '}
                        Products
                      </span>
                    </div>
          ...
          <div className="full-promo-overlay">
            <div className="container">
              <div className="full-promo-content">
                <span className="full-promo-text">
                  SHOP AND SAVE BIG ON HOTTEST PRODUCTS
                </span>
                <span className="full-promo-price">
                  STARTING AT <strong>₦7,999</strong>
                </span>
              </div>
            </div>
          </div>
